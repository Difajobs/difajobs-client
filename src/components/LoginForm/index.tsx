      ...
      <form onSubmit={formik.handleSubmit} className={styles.inputForm} >
        <Box className={styles.inputBox}>
          ...
          <TextField
            ...
            InputProps={{ style: { borderRadius: '20px', height: '52px', padding: '0px 5px' } }}
          />
          {formik.touched.email && formik.errors.email && (
            <Typography className={styles.errorMsg}>{formik.errors.email}</Typography>
          )}
        </Box>
        <Box className={styles.inputBox}>
          ...
          {formik.touched.password && formik.errors.password && (
            <Typography className={styles.errorMsg}>{formik.errors.password}</Typography>
          )}
        </Box>
        <Box className={styles.buttonBox}>
          <Button className={styles.button} type="submit" disabled={formik.isSubmitting}>
            {formik.isSubmitting ? <CircularProgress size={24} color="inherit" /> : 'Masuk'}
          </Button>
